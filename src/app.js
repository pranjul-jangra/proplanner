...
app.post('/login', async (req, res)=>{
    try{
        ...
        if(isVerified){
            ...
            res.cookie('proPlannerRefreshToken', refreshToken, {httpOnly: true, sameSite: 'Strict', maxAge: 15 * 24 * 60 * 60 * 1000});
...
app.patch('/home/settings/update-profile', async ( req, res )=>{
    try{
        ...
        res.cookie('proPlannerRefreshToken', newRefreshToken, {httpOnly: true, sameSite: 'Strict', maxAge: 15 * 24 * 60 * 60 * 1000 });
...
app.post('/home/settings/generate-otp', async (req,res)=>{
    try{
        ...
        if(forceOtpGeneration){
            res.clearCookie('otpToken', { httpOnly: true, sameSite: 'Strict' });  // marks the cookies to clear in the next request
            res.clearCookie('resendCountdown', { httpOnly: true, sameSite: 'Strict' }); //marks the cookies to clear in the next request

        }else if(req.cookies?.otpToken) {    // Checks for any active OTP
        ...
        res.cookie('otpToken', otpToken, {httpOnly: true, sameSite: 'Strict', maxAge: 15 * 60 * 1000});
        res.cookie('resendCountdown', resendCountdown, {httpOnly: true, sameSite: 'Strict', maxAge: 15 * 60 * 1000})
...
app.post('/home/settings/verify-otp', async (req, res) => {
    try {
      ...
      try {
        ...
        res.clearCookie('otpToken', { httpOnly: true, sameSite: 'Strict' });
        res.clearCookie('resendCountdown', { httpOnly: true, sameSite: 'Strict' });
...
app.post('/home/settings/logout', async (req, res)=>{
    try{
        ...
        res.clearCookie('proPlannerRefreshToken', { httpOnly: true, sameSite: 'Strict' })
